...
| `logger`           | the `rad` logger! a standard `Deno` logger with the commonplace log-level methods (e.g. `.info(...)`, `.debug(...)`, etc). see [the source](https://github.com/cdaringe/rad/blob/v8.0.3/src/logger.ts) |
...
exec deno run --allow-read --allow-write --allow-net --allow-env --allow-run --allow-hrtime 'https://raw.githubusercontent.com/cdaringe/rad/v8.0.3/src/bin.ts' "$@"
```
...
deno  \
  ...
  'https://raw.githubusercontent.com/cdaringe/rad/v8.0.3/src/bin.ts' \
  test
