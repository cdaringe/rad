...
| `logger`           | the `rad` logger! a standard `Deno` logger with the commonplace log-level methods (e.g. `.info(...)`, `.debug(...)`, etc). see [the source](https://github.com/cdaringe/rad/blob/v6.4.0/src/logger.ts) |
...
exec deno run --allow-read --allow-write --allow-net --allow-env --allow-run --allow-plugin --allow-hrtime --unstable 'https://raw.githubusercontent.com/cdaringe/rad/v6.3.5/src/bin.ts' "$@"
```
...
deno  \
  ...
  'https://raw.githubusercontent.com/cdaringe/rad/v6.3.5/src/bin.ts' \
  test
...
[deno debugging](https://deno.land/manual@v1.5.4/getting_started/debugging_your_code)
